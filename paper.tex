...
\shorttitle{SiIV Absorption from HD209458b}
...
** Explain that the photospheric emission is much smaller, which is important for limb brightening
...
\caption{(a) \siIV\ emission from the Solar limb as observed by SUMER (Solar Ultraviolet Measurement of Emitted Radiation) \citep{wiik} is strongly limb-brightened. We employ for \siIV\ emission, and whose light curve is given by  equation \ref{disk}.}
\label{limbs}
\end{center}
\end{figure}
...
Figure \ref{lightc} shows the light curve for \citet{vidmad}'s data and our best fit model. In addition to the best fit model, we show three more limb brightened models where \p\ is a fixed parameter for comparison. These include \p\ equal to 0, 0.12, and 0.32 representing no \siIV\ absorption, the optical radius (\citep{knutsonprop}) and the \hi\ radius\footnote{This was found by \citet{vidmad} for absorption of the wings of the \lya\ emission profile}, respectively. Our best fit model matches the \hi\ radius closely, representing absorption of \siIV\ by a Roche Lobe-sized atmosphere. 
...
R$_p$/R$_*$ = 0.34, close to the 0.32 found by \citet{vidmad} for the narrowband \lya\ detection. A
...
Limb brightening has important effects on transit light curves, especially when considering optically thin emission lines from the star. Our Double-U model fitted to the \siIV\ transit curve from \citet{vidmad} fits the data better than a non-detection, even when accounting for the additional degrees of freedom in the model. Our best fit model gives a \siIV\ transit depth of 11\% at the limb  of the star (-0.06\% + 0.09\%), which requires a layer of Si$^{2+}$ as large or even larger than the planets' Roche lobe. It is comparable to the \oi\ transit depth calculated by \citet{vidmad} and the \hi\ transit depth calculated by \citet{benjaf7}. As \citet{kosk} point out, a hard sphere is a poor approximation to atmospheric absorption and so the \siIV\ absorption profile probably extends beyond the predicted radius.

If the best-fit transit depth is true, it may support models with very high concentrations of ions in the atmosphere, ie. high metallicity, \citet{kosk}. If the metallicity is high, so must also be the temperature \citet{kosk}.

HD209458b's thermosphere is thought to be $\approx$ If significant \siIV\ absorption may favor high-metallicity models or high temperature models
	along with \cii\ \oi and \hi.

It's unlikely a big active region  could make both the Single-U \oi\ absorption and the Double-U \siIV\ emission {\it both} deep
