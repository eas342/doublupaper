...
\section{\siIV\ Absorption by HD 209458b}
...
Instead of the uniform disk, single-U model, we fit their \siIV\ 1394 \AA\ data (\citet{vidmad}, Figure 3) with a Double-U model as given in equation \ref{disk}. This model is appropriate for the \siIV\ emission, as evident in the Solar image in Figure \ref{limbs} (a) where the strong limb-brightening is apparent; we assume that HD209458 has a similar emission structure to the Sun because it is a type G0V star, close to Solar type.

The light curve for \citet{vidmad}'s data and our best fit models are shown in Figure \ref{lightc}. In addition to the best fit model, we show several different optically thin models where R$_p$/R$_*$ is a fixed parameter that is equal to 0 fit the data with an impact parameter of 0.48 R$_*$, using the an inclination of 86.9$^\circ$ \citep{knutsonprop} and a semi-major axis a=8.82 R$_*$. 
...
significant \siIV\ absorption may favor high-metallicity models or high temperature models
	along with \cii \oi and 

\section{Conclusion}

Spatial effects of the emission are as important as spatial effects on the planet\\
-Optically thin emission lines should be observed at the limb, not at phase=0 \\
	Note that $\delta \approx 1/2 (R_p/R_*)$ for an optically thin line at phase=0
