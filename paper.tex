...
For $1-p < x < 1+p$, the planet straddles the edge of the star, and
 for $0 < x < 1-p$, the planet is fully contained within the disk of the star. If the planet's radius is larger than the star's -- say, for instance, an M star is host to a giant planet with an inflated atmosphere, the formulae are the same as the straddled. The transit depth is 
% Explicitly,
...
\begin{align*}
&&0 < x < 1-p & & |1 - p| < x < 1+p & & 1+p < x ~ or ~ x < p - 1\\
