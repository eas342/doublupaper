...
We present in this paper a slightly more detailed model of optically thin stellar emission that includes flux from the central region of the stellar disk. Our model's transit depth scales as $(R_p/R_*)^{3/2}$, instead of $R_p/R_*$. In
...
Under this geometrically thin approximation, the total flux from the star is proportional to the surface area of the hemisphere facing the Earth, as pictured in Figure \ref{fig01} times its thickness $h$. The amount of emission that the planet blocks is then simply the amount of the surface that the planet covers times $h$. In this geometrical limit, therefore, the scale height of the chromosphere cancels out.
...
chromospheric wavelengths, as emphasized by \citet{assef}. It should also be noted that most limb brightened lines are in the ultraviolet, where planets absorb light from a larger radius than for optical and infrared wavelengths \citep{vidmad}. This means that transits of ultraviolet emission lines are boosted {\it doubly} over optical emission, once for the (\p )$^{1/2}$ scaling and once for the additional atmospheric absorption.
...
Figure \ref{fig02} (solid curve) shows the transit light curve (1-$\delta (x)$) for planet that has \p =0.08 using our thin-shell approximation.  The analytic estimate given by equation \ref{depth_anal} and the more detailed equations \ref{ingress} and \ref{disk} agree well, predicting maximum chromospheric depths to be 1.8 times
deeper than for uniform disk brightness.\footnote{For a uniform disk emission $\delta_{\mathrm{max}} $=$(\pi R_p / \pi R_*)^2$.} 
...
transit than for a uniform disk transit by a factor of
